<div class="p-4 sm:p-8 bg-white shadow sm:rounded-lg">
    <div class="flex items-start justify-between gap-4">
        <div>
            <h3 class="text-lg font-medium text-gray-900">{{ __('Database Overview') }}</h3>
            <p class="mt-1 text-sm text-gray-600">
                {{ __('Read-only schema and table monitoring for the current :driver connection.', ['driver' => strtoupper($databaseOverview['driver'])]) }}
            </p>
        </div>
        <a
    ...
    <div class="mt-4 flex flex-wrap items-center gap-x-6 gap-y-2 text-sm text-gray-600">
        <span><span class="font-medium text-gray-900">{{ __('Driver') }}:</span> {{ $databaseOverview['driver'] }}</span>
        <span><span class="font-medium text-gray-900">{{ __('Database') }}:</span> {{ $databaseOverview['database'] }}</span>
        ...
        <table class="min-w-full divide-y divide-gray-200 text-sm">
            <thead class="bg-gray-50">
                <tr>
                    ...
                    <th class="px-4 py-3 text-left font-semibold text-gray-700">{{ __('Rows') }}</th>
                    <th class="px-4 py-3 text-left font-semibold text-gray-700">{{ __('Metadata') }}</th>
                    <th class="px-4 py-3 text-left font-semibold text-gray-700">{{ __('View') }}</th>
                </tr>
            </thead>
